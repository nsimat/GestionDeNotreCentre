...
as
  begin
    if not exists(SELECT * FROM PERSONNE WHERE NumeroRegistre = @NumeroRegistre) 
	   insert into PERSONNE(NumeroRegistre, Nom, Prenom, Email, Rue, Ville, CodePostal, Pays, NumeroTelephone, CV, UserLogin, MotDePasse, IdEntreprise) 
	   values (@NumeroRegistre, @Nom, @Prenom, @Email, @Rue, @Ville, @CodePostal, @Pays, @NumeroTelephone, @CV, @UserLogin, @MotDePasse, @IdEntreprise);
	else 
